...
from modules import VectorQuantizedVAE, VQVAEMonster
...
parser.add_argument('--train_run', type=str, default='monster_crop_128_recon_1_0.5_0.5_bs_4')
...
#########################################################################
def random_crop_masha(batch, size_ = 128): #bs, 1, 1000, 512
    stack = []
    for img in batch:
...
#for fold, (train_idx,val_idx) in enumerate(splits.split(np.arange(50))):
    ...
    ####### model #######
    if cuda:
        model = VQVAEMonster(in_channel=1, channel=128, n_res_block=2, n_res_channel=32, embed_dim=64, n_embed=32, decay=0.99, #n_embed default 512
                 first_stride=4, second_stride=2).cuda()
    else:
        model = VQVAEMonster(in_channel=1, channel=128, n_res_block=2, n_res_channel=32, embed_dim=64, n_embed=32, decay=0.99, #n_embed default 512
        ...
        for phase in ['train', 'valid']:
            ...
            for i, (slice_, mask) in enumerate(loader): #slice bs,1000,512,1
                ...
                with torch.set_grad_enabled(phase == 'train'):
                        ...
                        out_crop,qq_t,qq_b = model(slice_crop)
                        print('out statistics ', out_crop.min(), out_crop.max(), out_crop.mean(), out_crop.std())

                        loss_recons = F.mse_loss(out_crop, slice_crop)
                        ...
                        loss = 50*loss_recons + loss_vq + 1.7 * loss_commit
                ...
                loader.set_description((f'phase: {phase}; epoch: {epoch + 1}; total_loss: {loss.item():.5f}; '
                                        ...
                                        f'lr: {lr:.5f}'))
                if i % 50 == 0:
                        recon_pydicom(n_row, sample_patch, model, f'{save_path}', epoch, Tensor)
