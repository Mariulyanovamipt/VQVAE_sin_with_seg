...
class Quantize(nn.Module):
    def __init__(self, dim, n_embed, decay=0.99, eps=1e-5):
        ...
        self.register_buffer('embed', embed) #save these parameters but not train in optimizer
    ...
    def forward(self, input):
        ...
        #print('EMBED IND SHAPE ---', embed_ind.shape)
...
class VQVAE1D(nn.Module):
    def __init__(self, in_channel=1, channel=128, n_res_block=2, n_res_channel=32, embed_dim=64, n_embed=512, decay=0.99,
        ...
        self.quantize_conv_t = nn.Conv2d(channel, embed_dim, 1)
        self.quantize_t = Quantize(embed_dim, n_embed)
        self.dec_t = Decoder1D(embed_dim, embed_dim, channel, n_res_block, n_res_channel, stride=second_stride)
        self.quantize_conv_b = nn.Conv2d(embed_dim + channel, embed_dim, 1)
        self.quantize_b = Quantize(embed_dim, n_embed)
    ...
    def encode(self, input):
        ...
        enc_b = self.enc_b(input)
        #print('shape b', enc_b.shape)

        enc_t = self.enc_t(enc_b)
        #print('shape t', enc_t.shape)

        quant_t = self.quantize_conv_t(enc_t).permute(0, 2, 3, 1)
        print('shape quant t', quant_t.shape)
        quant_t, diff_t, id_t = self.quantize_t(quant_t)
        quant_t = quant_t.permute(0, 3, 1, 2)
        diff_t = diff_t.unsqueeze(0)

        dec_t = self.dec_t(quant_t)
        #print('shape dec t b', dec_t.shape)
        #print('shape e b', enc_b.shape)
        enc_b = torch.cat([dec_t, enc_b], 1)
        #print('shape enc b', enc_b.shape)

        quant_b = self.quantize_conv_b(enc_b).permute(0, 2, 3, 1)
        #print('shape quant b', quant_b.shape)
        quant_b, diff_b, id_b = self.quantize_b(quant_b)
        quant_b = quant_b.permute(0, 3, 1, 2)
        diff_b = diff_b.unsqueeze(0)
    ...
    def decode(self, quant_t, quant_b):
        #print('AAAAA decoder')
        #print('quant t shape',  quant_t.shape)
        #print('quant b shape', quant_b.shape)
        upsample_t = self.upsample_t(quant_t)
        quant = torch.cat([upsample_t, quant_b], 1)
        #print('quant shape for dec', quant.shape)
        dec = self.dec(quant)
        #print('dec sh', dec.shape)
...
class Encoder1D(nn.Module):
    def __init__(self, in_channel, channel, n_res_block, n_res_channel, stride):
        ...
        if stride == 16:
            ...
            blocks = [
                nn.Conv2d(in_channel, channel // 2, (4,1), stride=2, padding=(1,0)),
                nn.LeakyReLU(inplace=True),
                nn.Conv2d(channel // 2, channel, (4,1), stride=2, padding=(1,0)),
                nn.LeakyReLU(inplace=True),
                nn.Conv2d(channel, channel, (3,1), padding=(1,0)),
            ...
            blocks = [
                nn.Conv2d(in_channel, channel // 2, (4,1), stride=2, padding=(1,0)),
                nn.LeakyReLU(inplace=True),
                nn.Conv2d(channel // 2, channel, (3,1), padding=(1,0)),
            ]
    ...
    def __init__(self, in_channel, out_channel, channel, n_res_block, n_res_channel, stride):
        ...
        if stride == 16:
            ...
            blocks.extend([
                nn.ConvTranspose2d(channel, channel // 2, (4,1), stride=2, padding=(1,0)),
                nn.LeakyReLU(inplace=True),
                nn.ConvTranspose2d(channel // 2, out_channel, (4,1), stride=2, padding=(1,0)),
            ])

        elif stride == 2:
            blocks.append(
                nn.ConvTranspose2d(channel, out_channel, (4,1), stride=2, padding=(1,0))
            )
